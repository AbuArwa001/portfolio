...
const useAuth = () => {
  ...
  useEffect(() => {
    ...
    const token = localStorage.getItem("authToken"); // Assuming you store user role

    setIsLoggedIn(!!token); // Adjust this based on your auth logic
    setIsLoading(false);
  }, []);
...
const data = {
  ...
  navSecondary: (isLoggedIn: boolean) => [
      ...
      : []),
    {
      ...
      url: "#",
      icon: IconSearch,
      component: <SearchButton searchData={searchData} />,
