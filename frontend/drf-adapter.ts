...
export class DRFAdapter implements Adapter {
  ...
  private async fetchDRF<T = unknown>(
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`DRF API error: ${response.statusText} - ${text}`);
      }

      // If response is empty but successful, return empty object
      if (!text.trim()) {
      ...
      console.error("fetchDRF error:", error);
      throw error;
    }
  }
